...
const createPost = async (req, res) => {
  try {
    const { title, content, userId,question } = req.body;
...
const getPosts = async (req, res) => {
  ...
  try {
    ...
    let { page, sortBy, author, search, liked ,question} = req.query;
    ...
    if (question){
        posts = posts.filter((post) => post.question == question);
}
    ...
    if (userId) {
      posts= await setLiked(posts, userId);
